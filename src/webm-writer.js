...
const WebMWriter = function (ArrayBufferDataStream, BlobBuffer) {
  return function (options) {
    let TARGET_CLUSTER_DURATION_MSEC = 5000, // may exceed this if added frames aren't keyframes
    ...
    function flushClusterFrameBuffer() {
      ...
      for (let i = 0; i < clusterFrameBuffer.length; i++) {
        rawImageSize +=
          clusterFrameBuffer[i].frameByteLength +
      ...
      for (let i = 0; i < clusterFrameBuffer.length; i++) {
        cluster.data.push(
          createContainerForFrame(
            clusterFrameBuffer[i],
            clusterFrameBuffer[i].frameType === "key"
          )
        );
      }
    ...
    function addChunkToCluster(chunk) {
      ...
      if (
        clusterDuration >= TARGET_CLUSTER_DURATION_MSEC &&
        chunk.frameType === "key"
      ) {
    ...
    function rewriteSegmentLength() {
      let buffer = new ArrayBufferDataStream(10),
        oldPos = blobBuffer.pos;
